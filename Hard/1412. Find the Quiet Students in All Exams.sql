...
with 
c1 as (select score, student_id, exam_id, 
       min(score) over (partition by exam_id) min_score, 
       max(score) over (partition by exam_id) max_score 
       from exam),
c2 as (select student_id from c1 where score=min_score or score=max_score)

select student_id, student_name from student 
where student_id not in (select distinct student_id from c2) 
