...
with 
t1 as(
select stock_name, sum(price) as buy_p, operation_day from stocks where operation='Buy' group by 1),
t2 as(
select stock_name, sum(price) as sell_p, operation_day from stocks where operation='Sell' group by 1)

select t1.stock_name as 'stock_name', (t2.sell_p-t1.buy_p) as 'capital_gain_loss'
