...
"2021-4-28" --> We have two transactions with IDs 5 and 9. The transaction with ID 5 has an amount of 40, 
                while the transaction with ID 9 has an amount of 21. 
...
WITH
t1 as (select transaction_id, 
      dense_rank() over (partition by DATE_FORMAT(day, "%Y-%m-%d") order by amount desc) as row_num 
      from transactions)

select transaction_id from t1 where row_num=1 order by transaction_id 
